/**
 * compendium.module.ts  (desu1 — updated)
 * -----------------------------------------------------------------
 * Angular module for Devil Survivor (DS1) and Overclocked (DSO).
 *
 * Changes vs the original:
 *   • Uses DesuCompendiumRoutingModule (DSO-scoped, adds /skill-recipe)
 *     instead of the shared smt4f CompendiumRoutingModule.
 *   • FormsModule added so [(ngModel)] works in SkillFusionGeneratorComponent.
 *   • SkillFusionGeneratorComponent declared here.
 */
...
function estimateDesuPrice(demon, statPrices: number[], skillPrices: { [skill: string]: number }): number {
  ...
  const skillPrice = Object.entries(skills).reduce(
    (acc, [sname, slvl]) => acc + (slvl < 2 ? skillPrices[sname] ?? 0 : 0), 0
  );
